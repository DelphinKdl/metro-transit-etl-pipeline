...
def transform_predictions(raw_predictions: List[Dict[str, Any]]) -> pd.DataFrame:
    ...
    # Deduplicate
    dedup_cols = [c for c in ["station_code", "line", "destination_code", "minutes_to_arrival", "extracted_at"] if c in df.columns]
...
def aggregate_station_metrics(df: pd.DataFrame) -> List[Dict[str, Any]]:
    ...
    aggregates["calculated_at"] = datetime.now(timezone.utc)
...
def enrich_with_metadata(
    ...
    """
    generated_run_id = run_id or datetime.now(timezone.utc).strftime("%Y%m%d%H%M%S")
